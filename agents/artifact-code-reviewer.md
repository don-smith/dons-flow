...
| Phase 4 §1 (foo-bridge.ts) | <n/a> | blocker | actionability | Module path `@myflow/foo/lib/bridge` does not exist; `@myflow/foo` has no `lib/` directory at HEAD | Add a Phase 0 that scaffolds `lib/` + registers it in `package.json` exports — name the scaffold phase, do not draft its contents |
