...
All notable changes to `@myflow/web-tools` are documented here.
...
### Added
- **You.com search+fetch provider** — new `FullProvider` backed by You.com's Search API (`POST /v1/search`) and Contents API (`POST /v1/contents`). Returns native markdown with `contentType: "text/markdown"`. Configure via `YOUCOM_API_KEY` env var or `apiKeys.youcom` in `~/.config/web-tools/config.json`. `web_fetch` uses the Contents API for clean markdown extraction (same path as Jina/Firecrawl).
...
### Changed
- Refresh npm cover (`docs/cover.{svg,png}`): align with the unified card layout used across the `@myflow/*` family and add a provider chip strip surfacing all six backends (Brave active, plus Tavily, Serper, Exa, Jina, Firecrawl) with the `/web-search-config` hint.
...
### Added
- Per-tool guidance overrides for `web_search` and `web_fetch` via `~/.config/web-tools/config.json`.
...
### Changed
- Consolidated into the `don-smith/myflow` monorepo. Version aligned to the @myflow/pi family lockstep starting point. No runtime behavior change from `0.1.2`.
