 ...
 *
 * Lives under the `@myflow/i18n/loader` subpath (separate from the
 * SDK entry) so a consumer doing `await import("@myflow/i18n/loader")`
 * does not pull `i18n-ui.ts` and the pi-tui dependency into its load graph
 * just to register strings.
 *
 * Soft-peer contract preserved: when `@myflow/i18n` is not installed
 ...
 *   caller's package, not from i18n.
 */
