...
beforeEach(() => {
	pkgDir = mkdtempSync(join(tmpdir(), "myflow-loader-"));
...
describe("registerLocalesFromDir", () => {
	...
	it("prefixes warn messages with the supplied label", () => {
		...
		registerLocalesFromDir("@test/ns", pkgUrl, { label: "myflow-test" });

		const messages = warn.mock.calls.map((c) => String(c[0]));
		expect(messages.length).toBeGreaterThan(0);
		expect(messages.every((m) => m.startsWith("myflow-test:"))).toBe(true);
		warn.mockRestore();
	});
	...
	it("defaults the warn label to the namespace when no label option is supplied", () => {
		...
		registerLocalesFromDir("@myflow/example", pkgUrl);

		const messages = warn.mock.calls.map((c) => String(c[0]));
		expect(messages.every((m) => m.startsWith("@myflow/example:"))).toBe(true);
		warn.mockRestore();
	});
});
