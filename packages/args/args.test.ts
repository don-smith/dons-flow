 ...
 * source after args switched away from filesystem-walking `loadSkills`
...
beforeEach(() => {
	tmpDir = mkdtempSync(join(tmpdir(), "args-"));
	skillCommands = [];
...
describe("handleInput — manifest-loaded skill baseDir (regression)", () => {
	...
	it("resolves ${SKILL_DIR} to dirname(filePath) even when sourceInfo.baseDir is the extension root", async () => {
		...
		// Pretend @myflow/pi (the extension) declared this skill via pi.skills.
	...
	// args must match exactly, or `${SKILL_DIR}` and the
