...
<div align="center">
  <a href="https://github.com/don-smith/myflow/tree/main/packages/args">
    <picture>
      <img src="https://raw.githubusercontent.com/don-smith/myflow/main/packages/args/docs/cover.png" alt="args cover" width="50%">
    </picture>
  </a>
</div>

Pass arguments to your skills like a shell command. `args` adds `$1`, `$ARGUMENTS`, `$@`, `${@:N}`, and `${@:N:L}` placeholders to [Pi Agent](https://github.com/badlogic/pi-mono) skills - write `/skill:deploy api production` and your skill body sees `$1` = `api`, `$2` = `production`. Skills without placeholders are untouched, so installing `args` is safe for any existing skill collection.

## Install

```bash
pi install npm:@myflow/args
```

Or run `/myflow-setup` if you have `@myflow/pi` installed.
...
args intercepts the `input` event (fires before Pi's built-in skill
...
args ignores this field - substitution is triggered by placeholders in the body, not the hint.
...
On Windows, args runs each command via `powershell.exe -Command` (PowerShell 5.1+ ships with every supported Windows version). On macOS / Linux it uses `sh -c`. Most POSIX utilities work on both platforms because PowerShell exposes them as aliases:
...
[![npm version](https://img.shields.io/npm/v/@myflow/args.svg)](https://www.npmjs.com/package/@myflow/args)
