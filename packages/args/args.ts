 ...
 * `_expandSkillCommand` is skipped — `args` is the only expander on that
...
function buildSkillIndex(pi: ExtensionAPI): Map<string, SkillIndexEntry> {
	...
	for (const cmd of pi.getCommands()) {
		...
		// (e.g. `packages/@myflow/pi`), not the skill folder. Pi's own internal
...
export function registerArgsHandler(pi: ExtensionAPI): void {
	...
	pi.on("session_start", (event) => {
		...
		// shaved that work but coupled this package to workflow's
