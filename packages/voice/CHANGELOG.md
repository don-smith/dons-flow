...
All notable changes to `@myflow/voice` are documented here.
...
### Changed
- Refresh npm cover (`docs/cover.{svg,png}`): align with the unified card layout used across the `@myflow/*` family and push the status row below the equalizer so the recording dot, timer, and hotkey hints no longer overlap the audio bars.
...
### Changed
- Published to npm — install via `pi install npm:@myflow/voice`.
...
- STT recognition failures logged to `~/.config/voice/errors.log` instead of being silently swallowed.
