/**
 * i18n bridge for voice — single thin import surface so every translation
 * call site routes through one place. Backed by `@myflow/i18n`'s SDK
 * when available; degrades to canonical-English fallbacks when not.
 *
 * - `t(key, fallback)` is `scope("@myflow/voice")` if the SDK is
 ...
 *   Returns `undefined` if i18n isn't installed or no locale is active —
...
export const I18N_NAMESPACE = "@myflow/voice";
