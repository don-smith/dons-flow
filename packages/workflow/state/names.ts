 ...
 * on failure; writers warn via `console.warn` with `[workflow]` prefix.
 */
...
export function readNamesIndex(cwd: string): NamesIndex | undefined {
	try {
		...
		console.warn(`[workflow] names index: ${e instanceof Error ? e.message : String(e)}`);
		return undefined;
	}
}
...
export function rebuildIndex(cwd: string): NamesIndex | undefined {
	try {
		...
		for (const fileName of entries) {
			...
			if (name) {
				...
				if (index[name] && index[name] !== runId) {
					console.warn(
						`[workflow] names index rebuild: duplicate name '${name}' claimed by runs ${index[name]} and ${runId} — keeping ${runId}`,
					);
				}
				index[name] = runId;
			}
		...
		console.warn(`[workflow] names index rebuild: ${e instanceof Error ? e.message : String(e)}`);
		return undefined;
	}
}
