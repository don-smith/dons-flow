...
function readJsonlRows<T>(cwd: string, runId: string, match: (row: unknown) => row is T): T[] {
	...
	try {
		...
		console.warn(`[workflow] workflow state: ${e instanceof Error ? e.message : String(e)}`);
		return [];
	}
	...
	for (const line of lines) {
		...
		try {
			...
			console.warn(
				`[workflow] workflow state: skipping malformed JSONL row — ${e instanceof Error ? e.message : String(e)}`,
			);
			continue;
		}
