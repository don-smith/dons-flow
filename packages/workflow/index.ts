/**
 * workflow — public API barrel for embedders. The Pi extension `default`
 * entry lives in the thin `./extension.ts` (not here); startup-time siblings
 * should import the runner-free `@myflow/workflow/registration` entry.
 *
 ...
 * like `@myflow/pi` opt in by calling `registerBuiltIns(...)`
 ...
 *      markdown-frontmatter parser live in `@myflow/pi`
 ...
 *      `@myflow/workflow/internal` (test-only — @myflow/pi's
 ...
 *      `RunState`.
 *
 * Per-module deep imports (`from "@myflow/workflow/api.js"`)
 ...
 * `@myflow/pi` along with the myflow-flavoured outcome
