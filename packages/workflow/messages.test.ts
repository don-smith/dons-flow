...
// in os.tmpdir() where bare specifiers like "@myflow/workflow" have
...
describe.skipIf(process.platform === "win32")("LEGACY_OVERLAY_NOTICE — embedded migration shell", () => {
	...
	beforeEach(() => {
		workDir = mkdtempSync(join(tmpdir(), "workflow-legacy-migrate-"));
	});
	...
	it("embedded shell migrates a legacy tree end-to-end", async () => {
		...
		const legacyRoot = join(workDir, ".workflow");
		...
		// notice re-emitted (the `existsSync(.workflow)` gate flips false).
	...
	it("embedded shell removes the legacy dir on a config-only (empty-packs) layout", () => {
		// A project that only ever wrote `.workflow/workflows.config.ts` —
...
describe.skipIf(process.platform === "win32")("LEGACY_RUNS_NOTICE — embedded migration shell", () => {
	...
	beforeEach(() => {
		workDir = mkdtempSync(join(tmpdir(), "workflow-legacy-runs-"));
	});
