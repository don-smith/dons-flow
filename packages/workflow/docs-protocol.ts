/**
 * System prompt protocol for workflow documentation delivery.
 *
 * Prepends a documentation reference block to the system prompt every turn
 * via `before_agent_start`, following the same pattern as args'
 * skill-invocation protocol (`packages/args/args.ts:439`). The block
 * tells the agent where to find workflow's authoring docs and when to
 ...
 * `SKILL_INVOCATION_PROTOCOL` in args).
 *
...
/**
 * Absolute path to the workflow package root. Resolved from this
 ...
 * (same posture as args' `SKILL_INVOCATION_PROTOCOL`).
 *
...
export function getDocsProtocol(): string {
	...
	protocolCache = [
		"",
		"workflow documentation (read only when the user asks about creating, modifying, or troubleshooting workflows):",
 ...
 *
 * Not exported as part of workflow's public type surface — internal
 ...
 * replacement clobbers prior extensions (args, @myflow/pi guidance, etc.).
 *
