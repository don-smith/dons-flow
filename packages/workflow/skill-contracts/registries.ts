...
const COMPARATORS_KEY = Symbol.for("@myflow/workflow:composition-comparators");
const DERIVERS_KEY = Symbol.for("@myflow/workflow:outcome-derivers");

/**
 * A callback that derives `OutputSpec` outcomes for `produces` stages from the
 * skill contract registry. Registered by a consumer (e.g. @myflow/pi) that owns
 ...
 * the first `/wf load`.
 *
 * @myflow/pi uses this to derive `rpivBucketOutcome(bucket)` from
