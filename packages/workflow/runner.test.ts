...
// Note: transcript-path scanning moved to @myflow/pi (`rpivArtifactCollector`)
...
// alongside it in `packages/@myflow/pi/extensions/core/`.
...
// ---------------------------------------------------------------------------
// Test fixture: a minimal phase-headings FanoutFn matching the @myflow/pi
// convention (`## Phase N:`). Mirrors what @myflow/pi declares inline in
// `built-in-workflows.ts` — kept local so workflow tests don't reach
// for an @myflow/pi import.
// ---------------------------------------------------------------------------
...
// paths (the @myflow/pi convention, inlined so this test file doesn't reach for
// an @myflow/pi import). Used as the default outcome on produces nodes
// built by `wf()` below.
// ---------------------------------------------------------------------------
...
describe("runWorkflow", () => {
	...
	beforeEach(() => {
		tmpDir = mkdtempSync(join(tmpdir(), "myflow-run-workflow-"));
	});
	...
	it("chains the second step on freshCtx — outer.newSession is called exactly once", async () => {
		...
		expect(chain.statusUpdates).toEqual([
			{ key: "workflow", value: "rpiv: stage 1/2 — research" },
			{ key: "workflow", value: "rpiv: stage 2/2 — design" },
	...
	it("records skipped + emits cancelled notification when outer newSession resolves cancelled", async () => {
		...
		expect(chain.statusUpdates.at(-1)).toEqual({ key: "workflow", value: undefined });
	});
	...
	// `_expandSkillCommand` is skipped, so `args` is the only expander.
	// If the skill isn't registered, `args` returns `{action:"continue"}`
	...
	describe("skill-registry pre-dispatch check", () => {
		let tmpDir: string;
		beforeEach(() => {
			tmpDir = mkdtempSync(join(tmpdir(), "myflow-skill-registry-"));
		});
		afterEach(() => {
			rmSync(tmpDir, { recursive: true, force: true });
		});
...
describe("totalStages denominator (countReachableNodes)", () => {
	...
	beforeEach(() => {
		tmpDir = mkdtempSync(join(tmpdir(), "myflow-total-stages-"));
	});
	afterEach(() => {
		rmSync(tmpDir, { recursive: true, force: true });
	});
	...
	it("counts every reachable node along a linear chain", async () => {
		writeFileSync(join(tmpDir, ".myflow-stub"), ""); // ensure cwd exists
		const chain = createMockSessionChain({
			cwd: tmpDir,
