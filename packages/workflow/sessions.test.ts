...
describe("sessions — validation retry loop", () => {
	...
	beforeEach(() => {
		tmpDir = mkdtempSync(join(tmpdir(), "myflow-sessions-retry-"));
	});
	afterEach(() => {
...
describe("sessions — outcome resolution", () => {
	...
	beforeEach(() => {
		tmpDir = mkdtempSync(join(tmpdir(), "myflow-sessions-outcome-"));
	});
	afterEach(() => {
		rmSync(tmpDir, { recursive: true, force: true });
	});
...
describe("sessions — collector ctx (always-unsliced branch + policy-derived offset)", () => {
	...
	beforeEach(() => {
		tmpDir = mkdtempSync(join(tmpdir(), "myflow-sessions-slice-"));
	});
	afterEach(() => {
		rmSync(tmpDir, { recursive: true, force: true });
	});
...
describe("sessions — spawn primitive", () => {
	...
	beforeEach(() => {
		tmpDir = mkdtempSync(join(tmpdir(), "myflow-sessions-spawn-"));
	});
	afterEach(() => {
		rmSync(tmpDir, { recursive: true, force: true });
	});
...
describe("sessions — success persistence", () => {
	...
	beforeEach(() => {
		tmpDir = mkdtempSync(join(tmpdir(), "myflow-sessions-success-"));
	});
	afterEach(() => {
		rmSync(tmpDir, { recursive: true, force: true });
	});
...
describe("sessions — halt routing", () => {
	...
	beforeEach(() => {
		tmpDir = mkdtempSync(join(tmpdir(), "myflow-sessions-halt-"));
	});
	afterEach(() => {
		rmSync(tmpDir, { recursive: true, force: true });
	});
...
describe("sessions — contract-sourced output validation", () => {
	...
	beforeEach(() => {
		tmpDir = mkdtempSync(join(tmpdir(), "myflow-sessions-contract-"));
	});
	afterEach(() => {
		rmSync(tmpDir, { recursive: true, force: true });
	});
