...
Complete reference for the `@myflow/workflow` authoring DSL. A workflow is a typed graph: named entry point, a `stages` record, and an `edges` table that maps each stage to another stage name, `"stop"`, or a predicate function.
...
```typescript
import { defineWorkflow } from "@myflow/workflow";
...
```typescript
import { produces, typeboxSchema } from "@myflow/workflow";
...
```typescript
import type { IterateFn } from "@myflow/workflow";
...
```typescript
import { acts } from "@myflow/workflow";
...
```typescript
import { terminal } from "@myflow/workflow";
...
```typescript
import { defineRoute } from "@myflow/workflow";
...
```typescript
import { defineCollector, opaque } from "@myflow/workflow";
...
```typescript
import { typeboxSchema } from "@myflow/workflow";
import { Type } from "@sinclair/typebox";
...
A full workflow with custom outcomes and conditional routing. This example uses only `@myflow/workflow` primitives — no external convention packages:
