...
All notable changes to `@myflow/warp` are documented here.
...
### Changed
- **Now publicly published on npm.** `package.json` no longer carries `"private": true`, so workspace publishes pick `@myflow/warp` up alongside the rest of the family. Install with `pi install npm:@myflow/warp`. The package is still opt-in: it is intentionally absent from `siblings.ts` and is NOT auto-installed by `/myflow-setup`.
