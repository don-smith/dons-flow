...
<div align="center">
  <a href="https://github.com/don-smith/myflow/tree/main/packages/warp">
    <picture>
      <img src="https://raw.githubusercontent.com/don-smith/myflow/main/packages/warp/docs/cover.png" alt="warp cover" width="50%">
    </picture>
  </a>
</div>

Native [Warp terminal](https://www.warp.dev/) toasts for [Pi Agent](https://github.com/badlogic/pi-mono) lifecycle events. When Pi finishes a long task, asks for your input, or completes a turn, `warp` emits Warp's `OSC 777` escape sequence and Warp surfaces a native OS notification. Outside Warp it does nothing - install it everywhere, it only fires where it's useful.
...
- **Configurable blocking-tool allowlist** - choose which tool calls flip the badge to **Blocked** via `~/.config/warp/config.json`. Defaults to `ask_user_question`.
...
```bash
pi install npm:@myflow/warp
```

`warp` is **opt-in** - it is NOT auto-installed by `/myflow-setup`. Install it explicitly only if you use Warp.
...
Optional file at `~/.config/warp/config.json`:
...
`warp` reads three environment variables Warp sets automatically:

- `TERM_PROGRAM === "WarpTerminal"` - required. Outside Warp the extension is a no-op.
- `WARP_CLI_AGENT_PROTOCOL_VERSION` - required for structured emission. If unset, `warp` does nothing rather than falling back to a less-rich legacy format.
...
`warp` is intentionally NOT registered in `@myflow/pi`'s sibling list. Not every Pi user uses Warp - auto-installing it everywhere would impose Warp-specific code on every install. If you don't use Warp, don't install it. If you do, install it explicitly. The package still joins the myflow lockstep version + shared release pipeline.

## License

[![npm version](https://img.shields.io/npm/v/@myflow/warp.svg)](https://www.npmjs.com/package/@myflow/warp)
