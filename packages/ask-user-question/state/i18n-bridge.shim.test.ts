 ...
 *   - bridge module (this directory) — `await import("@myflow/i18n")`
...
const SDK_SPECIFIER = "@myflow/i18n";

// The specifier may carry an optional subpath (e.g. `/loader`) — the entry
// point uses `@myflow/i18n/loader` to avoid pulling the UI module
// into the load graph; the bridge uses the bare `@myflow/i18n`
...
describe("i18n soft-peer shim — source shape", () => {
	it("bridge does not statically import the i18n SDK", () => {
		const src = readFileSync(BRIDGE, "utf8");
		expect(src).not.toMatch(STATIC_IMPORT);
	});

	it("bridge uses await import() for the i18n SDK", () => {
		const src = readFileSync(BRIDGE, "utf8");
	...
	it("entry point does not statically import the i18n SDK", () => {
		const src = readFileSync(ENTRY, "utf8");
		expect(src).not.toMatch(STATIC_IMPORT);
	});

	it("entry point uses await import() for the i18n SDK", () => {
		const src = readFileSync(ENTRY, "utf8");
		expect(src).toMatch(DYNAMIC_IMPORT);
	});
...
const MISSING_SDK_SPECIFIER = "@myflow/__definitely-not-installed__";
