 ...
 * SDK is actually installed. If `@myflow/i18n` is missing (standalone
 ...
 * `SUPPORTED_LOCALES` from the SDK. See `@myflow/i18n` README →
 * "Contributing translations" for the full convention.
 *
 * Extracted from @myflow/pi@7525a5d. Tool name "todo" and widget key
...
// Dynamic import keeps `@myflow/i18n` a soft optional peer: when the
...
try {
	const sdk = (await import("@myflow/i18n/loader")) as I18nLoader;
	sdk.registerLocalesFromDir(I18N_NAMESPACE, import.meta.url, { label: "todo" });
} catch {
