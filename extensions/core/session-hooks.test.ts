...
import { createMockCtx, createMockPi, stubGitExec } from "@myflow/test-utils";
...
beforeEach(() => {
	projectDir = mkdtempSync(join(tmpdir(), "myflow-session-"));
	clearInjectionState();
...
describe("session_start hook — notifications", () => {
	it("emits 'Copied N agents' info when added > 0", async () => {
		...
		expect(ctx.ui.notify).toHaveBeenCalledWith(expect.stringMatching(/Copied 2 @myflow/pi agent/), "info");
	});
	...
	it("warns about missing siblings with npm: prefix stripped", async () => {
		...
		vi.mocked(findMissingSiblings).mockReturnValueOnce([
			{ pkg: "npm:@myflow/advisor", matches: /./, provides: "x" },
			{ pkg: "npm:@myflow/args", matches: /./, provides: "y" },
		] as never);
		const { pi, captured } = createMockPi({ exec: stubGitExec({}) as never });
		registerSessionHooks(pi);
		const ctx = createMockCtx({ cwd: projectDir, hasUI: true });
		...
		expect(warnCall?.[0]).toContain("@myflow/pi: 2 sibling extensions missing");
		expect(warnCall?.[0]).toContain("@myflow/advisor");
		expect(warnCall?.[0]).toContain("@myflow/args");
		expect(warnCall?.[0]).toContain("Run /myflow-setup to install them.");
	...
	it("skips notifications when !hasUI", async () => {
		...
		vi.mocked(findMissingSiblings).mockReturnValueOnce([
			{ pkg: "npm:@myflow/todo", matches: /./, provides: "t" },
		] as never);
		const { pi, captured } = createMockPi({ exec: stubGitExec({}) as never });
		registerSessionHooks(pi);
...
describe("session_start hook — startup-maintenance-once latch", () => {
	...
	// Replaces the older cross-package coupling to workflow's child-session
	// Symbol.
...
describe("G0: session_start → real syncBundledAgents → notifyAgentSyncDrift", () => {
	...
	it("on a fresh tmp cwd, copies bundled agents and emits a single 'Copied N agents' info", async () => {
		...
		expect(existsSync(join(agentsDir, ".myflow-managed.json"))).toBe(true);
		expect(existsSync(join(agentsDir, ".myflow-managed.v2"))).toBe(true);

		const addedCalls = (ctx.ui.notify as ReturnType<typeof vi.fn>).mock.calls.filter(
			(c) => typeof c[0] === "string" && /Copied \d+ @myflow/pi agent/.test(c[0]),
		);
...
describe("before_agent_start hook", () => {
	...
	it("sets status to 'myflow: <name>' when prompt contains an owned @myflow/pi skill block", async () => {
		const { pi, captured } = createMockPi({
			exec: stubGitExec({ branch: "main", commit: "abc", user: "alice" }) as never,
		});
		registerSessionHooks(pi);
		...
		expect(ctx.ui.setStatus).toHaveBeenCalledWith("myflow-skill", "myflow: discover");
	});

	it("does not set status for a skill block whose name is not bundled with @myflow/pi", async () => {
		// Foreign / user-supplied skills must not be branded as myflow: — only names that
		// match a directory under packages/@myflow/pi/skills/ get the myflow-skill status.
		const { pi, captured } = createMockPi({
			exec: stubGitExec({ branch: "main", commit: "abc", user: "alice" }) as never,
		});
		registerSessionHooks(pi);
		const handler = captured.events.get("before_agent_start")?.[0];
		const ctx = createMockCtx({ cwd: projectDir });
		const skillPrompt = `<skill name="not-a-myflow-skill" location="/home/u/.pi/skills/not-a-myflow-skill">\nbody\n</skill>`;
		await handler?.({ prompt: skillPrompt } as never, ctx as never);
...
describe("agent_end hook", () => {
	it("clears the myflow-skill status", async () => {
		const { pi, captured } = createMockPi();
		registerSessionHooks(pi);
		const handler = captured.events.get("agent_end")?.[0];
		const ctx = createMockCtx();
		await handler?.({ messages: [] } as never, ctx as never);
		expect(ctx.ui.setStatus).toHaveBeenCalledWith("myflow-skill", undefined);
	});
});
