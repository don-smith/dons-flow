/**
 * Inject @myflow/pi's bundled skill contracts into the @myflow/workflow
 * registry. Mirrors register-built-in-workflows.ts: workflow is a SIBLING
 ...
 *
 * The framework is skill-agnostic; @myflow/pi (which already imports Pi's
 ...
 * `consumes.meta` / `produces.meta` in frontmatter — how @myflow/pi shapes its own
 ...
 * consumer registers it for; @myflow/pi wires it to "plans" today.
 */
...
/**
 * Scan @myflow/pi's bundled skills dir for `contract:` frontmatter blocks. Returns
...
export function buildSkillContractsFromFrontmatter(skillsDir: string): Array<[string, SkillContract]> {
	...
	try {
		skills = loadSkillsFromDir({ dir: skillsDir, source: "@myflow/pi" }).skills;
	} catch {
		return entries;
	}
...
export async function registerSkillContractsSource(): Promise<void> {
	try {
		...
		registerSkillContractsProvider(() => {
			...
			registerSkillContracts(buildSkillContractsFromFrontmatter(BUNDLED_SKILLS_DIR), "@myflow/pi");
		});
		...
		// `artifact-collector.js`, which value-imports `@myflow/workflow/registration`
		// at module-eval. A static edge would make THAT a load-time requirement of the
		// whole extension — so when the sibling is absent or not co-located, core
