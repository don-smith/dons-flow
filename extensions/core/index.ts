 ...
 *
 * Workflow runtime + `/wf` command live in `@myflow/workflow`. We
...
export default function (pi: ExtensionAPI) {
	...
	// current model UNCONDITIONALLY (independent of workflow), and the
	...
	// `input` + `agent_end` handlers are independent of workflow's
	...
	// `import("@myflow/workflow")`. Firing them concurrently makes jiti
	...
	const logRegistrationFailure = (label: string) => (err: unknown) =>
		console.error(`[core] failed to register ${label}:`, err);

	// Register the three workflow-dependent stacks STRICTLY in sequence — each
	// awaits the previous to settle so the concurrent `import("@myflow/workflow")`
