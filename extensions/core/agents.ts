 ...
 * Shared by session_start notifyCleanup and /myflow-update-agents handler so the
...
const MANIFEST_FILE = ".myflow-managed.json";
/**
 ...
 */
const V2_MARKER_FILE = ".myflow-managed.v2";
 ...
 * manual gate until a `/myflow-update-agents` run baselines the real hash.
 *
 ...
 * (notifyAgentSyncDrift, /myflow-update-agents) can surface it instead of
 ...
 *       file (.myflow-managed.v2) is written once committed and never re-fires
 ...
 *
 * apply=true (/myflow-update-agents): force adds/updates/removes regardless of
