 ...
 * static `import … from "@myflow/workflow"` in core/index.ts made
 * the whole extension fail to load when the (peerDependency) sibling was
 * absent, suppressing the /myflow-setup command + missing-sibling banner that
...
describe("registerBuiltInWorkflows", () => {
	it("registers all built-in workflows (six presets) when workflow is present", async () => {
		const { getBuiltIns, flushBuiltInProviders } = await import("@myflow/workflow/internal");
	...
	describe("when the workflow sibling is absent", () => {
		...
		it("no-ops without throwing and registers nothing", async () => {
			...
			vi.doMock("@myflow/workflow/startup", () => {
				throw Object.assign(new Error("Cannot find package '@myflow/workflow/startup'"), {
