/**
 * /myflow-setup — installs any SIBLINGS not present in the active Pi settings file
 * and prunes deprecated entries (e.g. the unscoped `npm:pi-subagents` from
 * the @myflow/pi 0.12.x → 0.14.0 line). Both mutations are previewed in the
...
const MSG_INTERACTIVE_ONLY = "/myflow-setup requires interactive mode";
const MSG_NOTHING_TO_DO = "All @myflow/pi sibling dependencies already installed.";
const MSG_CANCELLED = "/myflow-setup cancelled";
const MSG_CONFIRM_TITLE = "Apply @myflow/pi setup changes?";
...
function buildConfirmBody(missing: SiblingPlugin[], legacyEntries: string[]): string {
	const lines: string[] = ["@myflow/pi will apply the following changes:", ""];
	if (missing.length > 0) {
...
export function registerSetupCommand(pi: ExtensionAPI): void {
	pi.registerCommand("myflow-setup", {
		description: "Install @myflow/pi's sibling extension plugins",
