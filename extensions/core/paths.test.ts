...
describe("loadBundledSkillNames", () => {
	...
	it("returns the set of subdirectory names when the dir is readable", () => {
		const tmp = mkdtempSync(join(tmpdir(), "myflow-paths-"));
		try {
	...
	it("returns an empty set without throwing on a missing dir", () => {
		expect(loadBundledSkillNames("/nonexistent/@myflow/pi-skills-dir")).toEqual(new Set());
	});

	it("logs a [@myflow/pi]-prefixed warning that names the unreadable dir", () => {
		...
		loadBundledSkillNames("/nonexistent/@myflow/pi-skills-dir");
		expect(warnSpy).toHaveBeenCalledTimes(1);
		const [msg] = warnSpy.mock.calls[0]!;
		expect(msg).toMatch(/^\[@myflow/pi\]/);
		expect(msg).toContain("/nonexistent/@myflow/pi-skills-dir");
	});
});
