...
describe("/myflow-update-agents", () => {
	it("registers the command", () => {
		const { pi, captured } = createMockPi();
		registerUpdateAgentsCommand(pi);
		expect(captured.commands.has("myflow-update-agents")).toBe(true);
	});
	...
	it("UP_TO_DATE when no changes, no errors", async () => {
		...
		await captured.commands.get("myflow-update-agents")?.handler("", ctx as never);
