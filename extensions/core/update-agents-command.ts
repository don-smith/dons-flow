/**
 * /myflow-update-agents — apply-mode sync of bundled agents into ~/.pi/agent/agents/.
...
export function registerUpdateAgentsCommand(pi: ExtensionAPI): void {
	pi.registerCommand("myflow-update-agents", {
		description:
			"Sync @myflow/pi bundled agents into ~/.pi/agent/agents/: add new, update changed, remove stale. Also cleans up legacy per-project agent directories.",
		handler: async (_args, ctx) => {
			...
			// breaking the "/myflow-update-agents applies edits" promise
