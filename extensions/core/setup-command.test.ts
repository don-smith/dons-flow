...
describe("/myflow-setup — command shape", () => {
	it("registers under 'myflow-setup'", () => {
		const { pi, captured } = createMockPi();
		registerSetupCommand(pi);
		expect(captured.commands.has("myflow-setup")).toBe(true);
	});
});

describe("/myflow-setup — !hasUI", () => {
	it("notifies error and exits without inspecting siblings or settings", async () => {
		const { pi, captured } = createMockPi();
		registerSetupCommand(pi);
		const ctx = createMockCtx({ hasUI: false });
		await captured.commands.get("myflow-setup")?.handler("", ctx as never);
...
describe("/myflow-setup — nothing to do", () => {
...
describe("/myflow-setup — pre-confirm read-only contract", () => {
...
describe("/myflow-setup — user cancels", () => {
...
describe("/myflow-setup — post-confirm prune execution", () => {
...
describe("/myflow-setup — mixed success/failure report", () => {
...
describe("/myflow-setup — prune-only flow (no missing siblings)", () => {
