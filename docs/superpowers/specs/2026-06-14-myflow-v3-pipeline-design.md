...
Evolve myflow from a loose federation of three systems (Superpowers, RPIV, Don's closeout) into a single unified 5-stage pipeline with clear artifact handoffs, explicit skill assignments, and no ambiguous alternatives.
...
- `verification-before-completion` (myflow) — execution discipline: no success claim without fresh evidence
...
- `as-built-documentation` (myflow) — permanent record in `docs/changes/`

**Group 2 — Reflect & Reconcile:**
- `writing-retros` (myflow) — frozen retro in `docs/retros/`
- `capturing-learnings` (myflow) — promote twice-seen patterns
...
- `finishing-a-development-branch` (myflow) — merge/PR/cleanup decisions
...
| Security review (Land step 3) | myflow | RPIV's `code-review` already audits security alongside quality and dependencies |
...
- **myflow skills used:** verification-before-completion, epiphany-tabling, capturing-learnings, as-built-documentation, writing-retros, finishing-a-development-branch
